// The platform admin's session cookie primitives — split out from
// admin-session.ts specifically so lib/session.ts can import `isAdminSession`
// without a circular dependency (admin-session.ts itself imports HttpError
// FROM session.ts). This file imports nothing from session.ts.
//
